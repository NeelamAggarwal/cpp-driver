...
#ifndef __CASS_TOKEN_METADATA_HPP_INCLUDED__
#define __CASS_TOKEN_METADATA_HPP_INCLUDED__
...
class TokenMetadata {
public:
  static TokenMetadata* from_partitioner(const std::string& partitioner);

  virtual ~TokenMetadata() { }

  virtual void add_host(const Host::Ptr& host, const Value* tokens) = 0;
  virtual void update_host(const Host::Ptr& host, const Value* tokens) = 0;
  virtual void remove_host(const Host::Ptr& host) = 0;

  virtual void add_keyspaces(const VersionNumber& cassandra_version, ResultResponse* result) = 0;
  virtual void update_keyspaces(const VersionNumber& cassandra_version, ResultResponse* result) = 0;
